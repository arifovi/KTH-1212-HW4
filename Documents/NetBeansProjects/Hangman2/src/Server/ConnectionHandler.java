...
public class ConnectionHandler implements Runnable {
    ...
    private void writeToClient(SelectionKey key) throws IOException {
        if (isIdentical) {
            ...
            while (game.getCount() < 7 && game.underline.contains("_")) {
                message("Getting guess....");
                String gues = getInput();
                if (gues.isEmpty()) {
                    break;
                }
                state = game.updateUnderline(gues.toLowerCase().charAt(0));
                switch (state) {
                    case 0:
                        result = (MSG + game.underline + remainingTry + score);
                        message(result);
                        break;
                    case 1:
                        remainingTry -= 1;
                        result = ("\nWrong guess, try again\n" + game.underline + remainingTry + score);
                        break;
                    case 2:
                        remainingTry -= 1;
                        score -= 1;
                        result = ("\nYou loose! The word was: " + game.randomWord + "\n" + AGAIN + remainingTry + score);
                        break;
                    case 3:
                        score += 1;
                        result = ("\nCongrats! You won! The word was " + game.randomWord + "\n" + AGAIN + remainingTry + score);
                        break;
                    default:
                        break;
                }
    ...
    private void header(SelectionKey key) throws IOException {
        ...
        System.out.println("The word is: " + game.randomWord);
        socketChannel = (SocketChannel) key.channel();
        String message = MSG + game.underline + remainingTry + score;
